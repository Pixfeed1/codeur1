...
- Admin (26 septembre, inclus sans frais) : lien organisateur affiché sur la fiche projet (jeton déchiffré, pas régénéré) ; onglet Réglages > « E-mails automatiques » : les six e-mails organisateur sont des modèles avec variables (`DEFAULTS` dans `src/mailer.js`), personnalisables depuis l'admin (réglage `email_texts`, routes `/api/admin/emails`), aperçu rendu sans envoi (`previewAll`), retour au texte d'origine. Le lien du bouton n'est jamais éditable.
